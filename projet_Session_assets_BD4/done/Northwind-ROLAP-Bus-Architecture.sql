...
IF  EXISTS (SELECT name FROM sys.databases WHERE name = N'DWSession')
  BEGIN
     -- Close connections to the DWSession database 
    ALTER DATABASE [DWSession] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE
    DROP DATABASE [DWSession]
  END
GO


CREATE DATABASE DWSession
GO
